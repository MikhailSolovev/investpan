...
def build_env(
    ...
    if env_name in {"DownLinkEnv-v0", "DownLinkEnv-v1"}:
        if env_name == "DownLinkEnv-v0":
            ...
            env = DownLinkEnv(
                bs_n=4, ur_n=8, power=1.0, csi_noise_var=0.1, csi_clip=3.0
            )
    ...
    if env is None:
        try:
            env = deepcopy(env)
            print(
                f"| build_env(): Warning. NOT suggest to use `deepcopy(env)`. env_name: {env_name}"
            )
        except Exception as error:
            print(f"| build_env(): Error. {error}")
            raise ValueError(
                "| build_env(): register your custom env in this function."
            )
    return env
...
def get_gym_env_info(
    env, if_print
) -> (str, int, int, int, bool, float):  # [ElegantRL.2021.10.10]
    ...
    for i in range(2**10):
        ...
        for j in range(1):
            if agent is None:
                ...
                )  # if use 'with torch.no_grad()', then '.detach()' not need.
